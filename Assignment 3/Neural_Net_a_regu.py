...
def backward_propagation(X, Y, W, B, Z, A, batch_size, learning_rate, lamda):
    h = len(W)
    dz = Y - A[-1]
    for i in range(h - 1):
        dw = np.dot(A[-i-2].T, dz/batch_size) + (lamda/batch_size)*W[-i-1]
    ...
    dw = np.dot(X.T, dz/batch_size) + (lamda/batch_size)*W[0]
    db = np.sum(dz/batch_size, axis=0)
...
def validationNeural(X, Y, n_layers, h_layers, iters, b_size, learning_rate, lamda):
    accuracy_list = []
    accuracy_list2 = []
    error_list = []
    error_list2 = []
    ...
    while(p <= iters):
        ...
        for batch in batches:
            ...
            W, B = backward_propagation(x_mini, y_mini, W, B, Z, A, b_size, alpha, lamda)
...
if __name__ == "__main__":
    ...
    while(i <= 15):
        h_layers[0] = i
        print(h_layers)
        aa, ba, ce, de = validationNeural(X, Y, n_layers, h_layers, 6500, 100, 1, 0.01)
        a.append(aa)
        b.append(ba)
        c.append(ce)
        d.append(de)
...
#Gradient descent    
    ...
    while(i <= max_iter):
        ...
        for batch in batches:
            x_mini, y_mini = batch
            Z, A = forward_propagation(x_mini, W, B)
            W, B = backward_propagation(x_mini, y_mini, W, B, Z, A, b_size, alpha, 0.01)
